# You can copy the associated Python file 
...
class MASimilarBandsCrawling(tlp.ImportModule):
  def __init__(self, context):
    ...
    self.addIntegerParameter("max depth","Maximum level of recursion for crawling (distance to base band)","2")
    self.addStringParameter("logo folder path","Path for the folder in which the band logos are stored (leave blank to discard logos)","")
    self.base_url = ""
    self.min_score = 20
    self.max_depth = 2
    self.logo_folder = ""
  ...
  def crawlSimilarBands(self, bandNodes, url, current_level):
    ...
    for id_sim, url_sim, score_sim in band.similar_bands:
      if score_sim >= self.min_score and id_sim not in bandNodes.keys():
